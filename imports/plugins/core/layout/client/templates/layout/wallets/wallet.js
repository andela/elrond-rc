      ...
      Meteor.call(
        ...
        (err, res) => {
          if (res.error === "No recipient") {
            ...
            Alerts.toast(`An error occured, please try again. ${res.error}`, "error");
          }
        }
      );
    },
...
function handlePayment(result) {
  const type = "deposit";
  const transactionId = JSON.parse(result.reference).trxref;
